...
class TableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        APIManager.sharedInstance.postToService()
                .subscribe({
                    (response)  in
                    print(response)
                    if let statuse = Mapper<StackoverflowResponse>().map(JSONObject: response.element) {
                        print(statuse.items?[0].answerCount)
                        print(statuse.items?[0].creationDate)
                    }
